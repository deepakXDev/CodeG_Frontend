...
const AIReview = ({ code, language }) => {
    ...
    useEffect(() => {
        const getAIReview = async () => {
            if (!code.trim()) {
                // Don't show an error, just wait for code.
                return;
            }
            ...
            try {
                ...
                const data = await response.json();
                if (response.ok) {
                    setReview(data.review || 'No review feedback was generated.');
                } else {
                    throw new Error(data.error || 'Failed to get AI review');
                }
            } catch (err) {
                setError(err.message || 'Failed to connect to the AI review service.');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    // Custom components for dark-themed markdown
    const markdownComponents = {
        h1: ({ children }) => <h1 className="text-2xl font-bold text-white mt-6 mb-4 first:mt-0">{children}</h1>,
        h2: ({ children }) => <h2 className="text-xl font-semibold text-white mt-5 mb-3 first:mt-0">{children}</h2>,
        p: ({ children }) => <p className="text-gray-300 mb-3 leading-relaxed">{children}</p>,
        ul: ({ children }) => <ul className="list-disc text-gray-300 mb-4 space-y-2 ml-6">{children}</ul>,
        ol: ({ children }) => <ol className="list-decimal text-gray-300 mb-4 space-y-2 ml-6">{children}</ol>,
        li: ({ children }) => <li className="text-gray-300 leading-relaxed">{children}</li>,
        code: ({ inline, children }) => {
            if (inline) {
                return <code className="bg-gray-700 text-purple-300 px-1.5 py-0.5 rounded text-sm font-mono border border-gray-600">{children}</code>;
            }
            return <code className="block bg-[#1A1A1A] text-gray-300 p-4 rounded-lg overflow-x-auto text-sm font-mono whitespace-pre border border-gray-700">{children}</code>;
        },
        pre: ({ children }) => <pre className="bg-[#1A1A1A] rounded-lg overflow-x-auto mb-4 border border-gray-700">{children}</pre>,
        blockquote: ({ children }) => <blockquote className="border-l-4 border-purple-500 pl-4 italic text-gray-400 mb-3 bg-gray-800/50 py-2 rounded-r">{children}</blockquote>,
        ...
        return (
            <div className="text-center py-8">
                <div className="inline-flex items-center gap-3">
                    ...
                    <span className="text-gray-400">Analyzing your code...</span>
                </div>
                <p className="text-gray-500 text-sm mt-2">This may take a few moments.</p>
            </div>
        );
