...
console.log("Request URL:", `${compilerUrl}/submission/run-sample`);
  ...
  return (
    <div>
      ...
      <OnlineCompilerSection
        // Pass all the necessary state and handlers for the compiler
        isAuthenticated={isAuthenticated}
        selectedLanguage={selectedLanguage}
        handleLanguageChange={handleLanguageChange}
        languageOptions={languageOptions}
        handleAIFeature={handleAIFeature}
        runCode={runCode}
        isRunning={isRunning}
        code={code}
        setCode={setCode}
      ...
      {!isAuthenticated && <CommunityStatsSection />}
  ...
  <div className="relative pt-20 pb-16 overflow-hidden">
    ...
    <div className="relative container mx-auto px-6 text-center">
      ...
      <p className="text-xl text-gray-600 mb-8 max-w-3xl mx-auto">
        Master coding interviews with our comprehensive platform. Practice problems, get AI-powered feedback, and build the skills top companies are looking for.
      </p>
...
// 3. Online Compiler Section Component
const OnlineCompilerSection = ({
  isAuthenticated,
  selectedLanguage,
...
// Sub-components for the Compiler Section
const CompilerHeader = ({ selectedLanguage, handleLanguageChange, languageOptions, isAuthenticated, handleAIFeature, runCode, isRunning }) => (
      ...
      <div className="flex items-center space-x-2">
        {isAuthenticated && (
          <div className="flex space-x-2 mr-4">
            <button onClick={() => handleAIFeature("Hint")} className="bg-blue-500 text-white hover:bg-blue-600 px-3 py-2 rounded-lg transition-colors text-sm">Hint</button>
            <button onClick={() => handleAIFeature("Feedback")} className="bg-green-500 text-white hover:bg-green-600 px-3 py-2 rounded-lg transition-colors text-sm">Feedback</button>
            <button onClick={() => handleAIFeature("Explain")} className="bg-purple-500 text-white hover:bg-purple-600 px-3 py-2 rounded-lg transition-colors text-sm">Explain</button>
            <button onClick={() => handleAIFeature("Complexity")} className="bg-yellow-400 text-white hover:bg-yellow-500 px-3 py-2 rounded-lg transition-colors text-sm">Complexity</button>
          </div>
        )}
        <button
          onClick={runCode}
...
const CodeEditorPanel = ({  getMonacoLanguage, selectedLanguage, code, setCode, isAuthenticated, customInput, setCustomInput }) => (
...
const OutputAndAIPanel = ({ isAuthenticated, activeTab, setActiveTab, customInput, setCustomInput, problemDescription, setProblemDescription, constraints, setConstraints, selectedFeature, handleAIRequest, output, setShowLoginModal }) => (
  <div className="space-y-4">
    {isAuthenticated ? (
      <AuthenticatedUserOutputPanel
        activeTab={activeTab}
        setActiveTab={setActiveTab}
        customInput={customInput}
        setCustomInput={setCustomInput}
      ...
      <UnauthenticatedUserOutputPanel output={output} setShowLoginModal={setShowLoginModal} />
    )}
  </div>
);

// const AuthenticatedUserOutputPanel = ({ activeTab, setActiveTab, customInput, setCustomInput, problemDescription, setProblemDescription, constraints, setConstraints, selectedFeature, handleAIRequest, output }) => (
//   <>
//     <div className="flex border-b-2 border-gray-200">
//       <button onClick={() => setActiveTab("Input")} className={`px-4 py-2 font-medium ${activeTab === "Input" ? "text-blue-600 border-b-2 border-blue-600 bg-blue-50" : "text-gray-600 hover:text-gray-800"}`}>Input</button>
//       <button onClick={() => setActiveTab("Output")} className={`px-4 py-2 font-medium ${activeTab === "Output" ? "text-blue-600 border-b-2 border-blue-600 bg-blue-50" : "text-gray-600 hover:text-gray-800"}`}>Output</button>
//     </div>
//     <div className="space-y-4">
//         {/* ... (rest of the authenticated panel logic) */}
//     </div>
//   </>
// );

// const UnauthenticatedUserOutputPanel = ({ output, setShowLoginModal }) => (
//     <>
//         <div className="bg-white rounded-lg border-2 border-gray-300">
//             {/* ... (output panel) */}
//         </div>
//         <div className="bg-white rounded-lg border-2 border-gray-300">
//             {/* ... (AI features panel with login prompt) */}
//         </div>
//     </>
// );


// This component now contains the full logic for the tabbed view for logged-in users.
const AuthenticatedUserOutputPanel = ({ activeTab, setActiveTab, customInput, setCustomInput, problemDescription, setProblemDescription, constraints, setConstraints, selectedFeature, handleAIRequest, output }) => (
  <div className="space-y-4">
    <div className="flex border-b-2 border-gray-200">
      <button onClick={() => setActiveTab("Input")} className={`px-4 py-2 font-medium ${activeTab === "Input" ? "text-blue-600 border-b-2 border-blue-600 bg-blue-50" : "text-gray-600 hover:text-gray-800"}`}>Input</button>
    ...
    {activeTab === "AI" && (
      <button
        ...
        className={`mt-2 w-full py-2 rounded-lg hover:bg-black ${selectedFeature === "Hint" ? "bg-blue-500 text-white" : selectedFeature === "Feedback" ? "bg-green-500 text-white" : selectedFeature === "Explain" ? "bg-purple-500 text-white" : selectedFeature === "Complexity" ? "bg-yellow-500 text-white" : "bg-gray-300 text-gray-700"}`}
      >
        Get {selectedFeature}
      </button>
    )}
    ...
    {activeTab === "Output" && (
      <div className="bg-white rounded-lg border-2 border-gray-300">
        ...
        <div className="p-4 min-h-[150px]">
          <pre className={`font-mono text-sm whitespace-pre-wrap ${output.includes("Error") ? "text-red-600" : "text-black"}`}>
...
// This component now contains the full layout for logged-out users.
const UnauthenticatedUserOutputPanel = ({ output, setShowLoginModal }) => (
  <>
    <div className="bg-white rounded-lg border-2 border-gray-300">
      <div className="bg-gray-50 px-4 py-2 border-b-2 border-gray-200 rounded-t-lg">
    ...
    <div className="bg-white rounded-lg border-2 border-gray-300">
      ...
      <div className="p-4 grid grid-cols-2 gap-2">
        <button onClick={() => setShowLoginModal(true)} className="px-3 py-2 bg-blue-100 text-blue-800 rounded-lg hover:bg-blue-200 transition-colors text-sm font-medium border border-blue-300">Hint</button>
        <button onClick={() => setShowLoginModal(true)} className="px-3 py-2 bg-green-100 text-green-800 rounded-lg hover:bg-green-200 transition-colors text-sm font-medium border border-green-300">Feedback</button>
      </div>
...
// 4. Community Stats Section Component
const CommunityStatsSection = () => (
  <div className="py-20 bg-white">
    <div className="max-w-4xl mx-auto px-6 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <StatItem count="1000+" label="Problems Available" />
        <StatItem count="50K+" label="Solutions Submitted" />
        <StatItem count="10K+" label="Active Users" />
      </div>
    </div>
  </div>
);
...
// 5. Call To Action (CTA) Section Component
...
const LoginModal = ({ showLoginModal, setShowLoginModal, selectedFeature, navigate }) => {
