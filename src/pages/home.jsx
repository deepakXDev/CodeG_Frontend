...
const FEATURES_LIST = [
  {
    ...
    description:
      "Go beyond theory. Solidify your understanding of core algorithms and data structures with our expansive library of curated problems.",
  },
  {
    title: "High-Performance Web IDE",
    description:
      "Experience a seamless coding environment. Our powerful, multi-language IDE is designed for rapid development and testing.",
  },
  {
    title: "AI-Powered Interview Prep",
    description:
      "Gain a competitive advantage. Leverage AI-driven feedback and analysis to refine your solutions and master interview patterns.",
  },
];
  ...
  const runCode = useCallback(async () => {
    ...
    try {
      ...
      if (result.code !== 0 && result.stderr) {
        ...
        const stderrContent =
          result.stderr || result.error || "An unknown error occurred";
...
const HeroSection = ({
  ...
  <div className="relative text-center py-20 sm:py-32 px-4 overflow-hidden">
    ...
    <h1 className="text-4xl sm:text-6xl font-bold text-white mb-6 leading-tight">
      Build Your Edge.{" "}
      <span className="block text-gray-400">Ace the Interview.</span>
    </h1>
    <p className="text-lg sm:text-xl text-gray-400 mb-8 max-w-3xl mx-auto">
      Level up with a suite of powerful tools designed for elite software
      engineers. Tackle complex problems, receive instant AI-driven insights,
      and demonstrate the skills that define top-tier talent.
    </p>
    <div className="flex flex-col sm:flex-row gap-4 justify-center">
      <button
        onClick={handleGetStarted}
        className="bg-white text-black font-semibold py-3 px-8 rounded-lg hover:bg-gray-200 transition-colors shadow-lg"
      >
      ...
      <button
        onClick={onScrollToCompiler}
        className="bg-[#282828] text-white font-semibold py-3 px-8 rounded-lg border border-gray-700 hover:bg-[#3c3c3c] transition-colors"
      >
        Launch Web IDE
      </button>
    </div>
    ...
    <div className="container mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {features.map((feature) => (
          <div
            key={feature.title}
            className="bg-[#282828] p-8 rounded-lg border border-gray-700 hover:border-gray-500 transition-colors"
          >
...
const OnlineCompilerSection = React.forwardRef(
  (
    {
      isAuthenticated,
      editorState,
    ...
    <div id="compiler" ref={ref} className="py-20 px-4">
      <div className="container mx-auto text-center mb-12">
        <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">
          Instant Execution, Seamless Workflow
        </h2>
        <p className="text-lg text-gray-400">
          Our integrated development environment is ready for your most complex
          solutions. Write, test, and iterate in a frictionless environment.
        </p>
      </div>
      <div className="max-w-7xl mx-auto bg-[#282828] rounded-lg border border-gray-700 shadow-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 p-4">
          <div className="lg:col-span-3 h-[60vh] min-w-0">
            <Editor
              theme="vs-dark"
              language={getLanguageConfig(editorState.language).monaco}
              value={editorState.code}
              onChange={(value) =>
                setEditor((e) => ({ ...e, code: value || "" }))
              }
              options={{
          ...
          <div className="lg:col-span-2 flex flex-col gap-4 h-[60vh] min-w-0">
            <div className="flex-1 flex flex-col min-h-0">
              <h3 className="text-sm font-medium text-white px-3 py-2 bg-[#1A1A1A] rounded-t-md">
                Custom Input
              </h3>
              <textarea
                value={editorState.customInput}
                onChange={(e) =>
                  setEditor((es) => ({ ...es, customInput: e.target.value }))
                }
                placeholder="Enter input..."
                className="w-full flex-grow bg-[#282828] text-gray-300 p-3 rounded-b-md resize-none focus:outline-none font-mono text-sm border-t-0 border border-gray-700 overflow-auto"
              />
            </div>
            <div className="flex-1 flex flex-col min-h-0">
              <h3 className="text-sm font-medium text-white px-3 py-2 bg-[#1A1A1A] rounded-t-md">
              ...
              <pre className="text-sm text-gray-300 p-3 overflow-auto whitespace-pre font-mono flex-grow min-h-0 bg-[#282828] rounded-b-md border-t-0 border border-gray-700">
            ...
            {!isAuthenticated && (
              <div className="bg-[#282828] p-4 rounded-lg border border-gray-700 text-center">
                ...
                <p className="text-sm text-gray-400 mb-4">
                  Sign in to get hints, feedback, and code explanations.
                </p>
                <button
                  onClick={() => onAIFeatureClick("Login")}
                  className="bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg w-full hover:bg-purple-500 transition-colors"
                >
                  Sign In to Use AI
                </button>
              </div>
...
const CompilerHeader = ({
  ...
  <div className="flex justify-between items-center px-4 py-2 border-b border-gray-700">
    ...
    <button
      onClick={runCode}
      disabled={isRunning}
      className="text-sm bg-green-600 hover:bg-green-500 disabled:bg-green-900 disabled:text-gray-400 text-white font-semibold px-4 py-2 rounded-md transition"
    >
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-sm">
      <div className="bg-[#282828] rounded-lg p-8 max-w-md w-full mx-4 border border-gray-700 shadow-xl text-center">
        ...
        <p className="text-gray-400 mb-6">
          To access the **{feature}** tool and get a personalized coding
